...
if [[ ! -f "$RUN_DIRECTORY/search_manifest.json" || ! -f "$RUN_DIRECTORY/search_plan.json" || ! -f "$RUN_DIRECTORY/checkpoint.json" ]]; then
  print -u2 -- "ERROR: manifest、plan、checkpointのどれかがありません。"
  exit 1
fi

.venv/bin/python - "$RUN_DIRECTORY/search_manifest.json" <<'PY'
import json
import sys
from pathlib import Path

manifest = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
if manifest.get("schema_version") != "fatigue_sigma_auto_search_manifest_v1":
    raise SystemExit("ERROR: run manifest schemaが一致しません。")
PY

print -- "Project: $PROJECT_DIRECTORY"
print -- "Python: $(.venv/bin/python --version 2>&1)"
print -- "Run directory: $RUN_DIRECTORY"
print -- "実行予定session数: $(.venv/bin/python -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["planned_session_runs"])' "$RUN_DIRECTORY/search_plan.json")"
