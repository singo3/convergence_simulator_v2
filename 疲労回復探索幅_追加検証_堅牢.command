...
print -- "Preset: robust"
print -- "Actual simulation sessions: 54,000 (autonomous 43,200 + shared random 10,800)"
...
print -- ""
read -r "CONFIRMATION?堅牢追加検証をローカルCPUで開始するには ROBUST と入力してください: "
if [[ "$CONFIRMATION" != "ROBUST" ]]; then
  print -- "キャンセルしました。"
  exit 0
fi

OUTPUT_BASE="artifacts/fatigue_recovery_sigma_factorial"
CAPTURE_BASE="artifacts/fatigue_recovery_sigma_factorial-launcher-logs"
mkdir -p "$OUTPUT_BASE" "$CAPTURE_BASE"
CAPTURE_PATH="$CAPTURE_BASE/robust-$(date -u +%Y%m%dT%H%M%SZ)-$$.log"
