...
if manifest.get("schema_version") != "adaptive_placebo_validation_manifest_v1":
    raise SystemExit("ERROR: validation manifest schemaが一致しません。")
PY

print -- "Project: $PROJECT_DIRECTORY"
print -- "Python: $(.venv/bin/python --version 2>&1)"
print -- "Run directory: $RUN_DIRECTORY"
print -- "実行予定session数: $(.venv/bin/python -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["planned_session_runs"])' "$RUN_DIRECTORY/checkpoint.json")"
