...
def _state_map(
    ...
    ids = tuple(value)
    expected_ids = tuple(
        digital_life_config_for_role(role).digital_life_id for role in ("red", "green", "blue")
    )
    if len(ids) != 3 or set(ids) != set(expected_ids):
        raise ValueError("experimental outcome requires the fixed red/green/blue life roster")
    raw = {
        life_id: state.to_dict() if isinstance(state, RelationMemoryPersistentState) else state
...
class ExperimentalSessionOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        if self.adoption_result is not None and self.adoption_result not in ADOPTION_RESULTS:
        ...
        fatigue = _mapping_records("fatigue_trajectory_by_life", self.fatigue_trajectory_by_life)
        sigma = _mapping_records("sigma_trajectory_by_life", self.sigma_trajectory_by_life)
        ...
        if self.engine_completed:
            ...
            for life_id, before in initial.items():
                ...
                if audit.get("digital_life_id") != life_id:
                    raise ValueError("fatigue record Digital Life ID differs from its mapping key")
                if audit.get("e_at_session_start") != before.e:
                    raise ValueError(
                        "fatigue record session-start E differs from persistent-state handoff chain"
                    )
                ...
                if isinstance(after_policy, bool) or not isinstance(after_policy, (int, float)):
                ...
                if self.reference_arm:
                    ...
                    policy_version = audit.get("session_end_policy_version")
                    if policy_version == UNSELECTED_FULL_RECOVERY_POLICY_VERSION:
                        ...
                        if recovered:
                            raise ValueError("gradual-reference-only policy cannot fully recover")
                        if after.e != audit.get("e_before_session_end_policy"):
                            raise ValueError("gradual-reference-only policy must retain closing E")
                    else:
                        raise ValueError(
                            "experimental session-end fatigue policy is not recognized"
                        )
                expected_anchor = (
                    after.k_anchor
                ...
                if after.k_anchor != expected_anchor:
                    raise ValueError("final k_anchor differs from the candidate adoption result")
                if (
                    life_id == holder_id
            ...
            if sum(trial_deltas.values()) != int(self.candidate_generated):
                raise ValueError("trial_count increments must exactly match candidate generation")
            if self.candidate_generated and (holder_id is None or trial_deltas[holder_id] != 1):
                raise ValueError("only the holder may increment trial_count")
            if holder_id is not None:
                holder_final = final[holder_id]
                if k != holder_final.k_anchor:
                    raise ValueError("holder_final_k_anchor differs from final persistent state")
                if k is not None:
            ...
            for life_id, audit in sigma.items():
                if audit.get("digital_life_id") != life_id:
                    raise ValueError("sigma record Digital Life ID differs from its mapping key")
                if audit.get("candidate_generated") != self.candidate_generated:
                    raise ValueError("sigma record candidate flag differs from the outcome")
                if audit.get("candidate_accepted") != self.candidate_accepted:
                    raise ValueError("sigma record adoption flag differs from the outcome")
                audit_reference = audit.get("reference_arm")
                ...
                if not math.isclose(
                    ...
                    numeric["reference_sigma_at_w"] * numeric["sigma_multiplier"],
                    rel_tol=0.0,
                    abs_tol=1.0e-15,
                ):
                ...
                if self.candidate_generated != all(value is not None for value in movement):
                    raise ValueError("sigma candidate movement must exist exactly when generated")
                if self.candidate_generated:
                    ...
                    if self.candidate_accepted and any(
                        ...
                        raise ValueError("accepted final k_anchor differs from the sigma candidate")
        if self.schema_version != FATIGUE_SIGMA_SESSION_OUTCOME_SCHEMA_VERSION:
            raise ValueError(
                f"schema_version must be {FATIGUE_SIGMA_SESSION_OUTCOME_SCHEMA_VERSION}"
            )
        for name, value in {
            "session_index": index,
            "local_time_us": local,
            ...
            "holder_W_trial_1": _optional_number("holder_W_trial_1", self.holder_W_trial_1),
            "holder_W_trial_2": _optional_number("holder_W_trial_2", self.holder_W_trial_2),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "fatigue_trajectory_by_life": {
                life_id: dict(record) for life_id, record in self.fatigue_trajectory_by_life.items()
            },
            "sigma_trajectory_by_life": {
                life_id: dict(record) for life_id, record in self.sigma_trajectory_by_life.items()
            },
            "bundle_presentations": [record.to_dict() for record in self.bundle_presentations],
...
def _reference_fatigue_record(
    ...
    closing = [record for record in rounds if record.closing_evaluation_attribution]
...
def experimental_session_outcome_from_simulation(
    ...
    *,
    ...
    all_finalized = all(
        isinstance(state, RelationMemoryPersistentState) for state in finalized.values()
    )
    final = (
        {
            life_id: state
    ...
    try:
        ...
        projection_errors.append(f"bundle_presentation_error:{type(exc).__name__}:{exc}")
            ...
            if isinstance(component, ExperimentalAdaptiveConnectedDigitalLifeComponent):
                ...
                if decision_record is not None:
                    ...
                    sigma[life_id] = {
                        ...
                        "candidate_accepted": (
                            component.relation_memory_session_state().adoption_result == "accepted"
                        ),
    ...
    return ExperimentalSessionOutcome(
        ...
        adoption_result=(None if holder_session is None else holder_session.adoption_result),
        candidate_generated=(
        ...
        bundle_presentations=presentations,
        session_digest=(simulation.engine.deterministic_digest() if engine_completed else None),
    )
